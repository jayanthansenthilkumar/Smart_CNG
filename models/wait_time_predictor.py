...
class WaitTimePredictor:
    def __init__(self):
        # Initialize model/scaler if sklearn is installed, otherwise fallback to heuristic
        self.model = RandomForestRegressor(n_estimators=100, max_depth=10, random_state=42) if RandomForestRegressor is not None else None
    ...
    def train_from_csv(self, file_path: str):
        ...
        if pd is None or RandomForestRegressor is None:
            # Cannot train without pandas or sklearn installed
            print('Warning: pandas or scikit-learn not available; skipping training')
            return False
