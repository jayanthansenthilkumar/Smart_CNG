...
class LocationOptimizer:
    def __init__(self, data_file_path: str = None):
        ...
        # Use sklearn scaler if available, otherwise keep None
                ...
                with open(file_path, newline='', encoding='utf-8') as fh:
                    ...
                    for row in reader:
                        ...
                        station = {
                            ...
                            'lat': lat,
                            'lng': lng,
                            'morning_arrivals': float(row.get('demo_arrivals_per_hr_morning') or 0),
                            'evening_arrivals': float(row.get('demo_arrivals_per_hr_evening') or 0),
                            'overall_arrivals': float(row.get('demo_overall_arrivals_per_hr') or 0),
                            'service_time': float(row.get('demo_avg_service_time_min') or 0),
                            'servers': int(row.get('demo_servers_disp') or 1),
                            'rush_pattern': row.get('demo_rush_pattern', 'Steady'),
                            'wait_time_morning': self._safe_float(row.get('Wq_morning_min') or 0),
                            'wait_time_evening': self._safe_float(row.get('Wq_evening_min') or 0),
                            'wait_time_overall': self._safe_float(row.get('Wq_overall_min') or 0),
                            'total_station_time': self._safe_float(row.get('Expected_total_station_time_min') or 0),
                            'utilization': 0.0
                        }
                        self.existing_stations.append(station)
